...
The benchmark datasets are synthetic streams generated by `CsvDatasetGenerator`; they are useful for repeatable comparisons and regression checks, but they do not claim to reproduce all live-market characteristics such as exchange-specific burst patterns, auction transitions, news shocks, correlated cancels, or real queue-position behavior.
...
The optimized structure should benefit further from denser price distributions. When active levels cluster into fewer chunks, the chunk directory and bitmap traversal touch fewer cache lines and fewer active chunk words. Sparse distributions spread levels across more chunks, increasing directory and bitmap-walk overhead; dense books make the chunked array design closer to direct array access over adjacent levels.
